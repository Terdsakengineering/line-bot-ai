...
function startQuoteFlow() {
  return {
    replyMessages: [
      ...
      textMessage(
        'ขั้นตอนที่ 1: รบกวนส่งไฟล์แบบงานมาได้เลยค่ะ (รูปภาพหรือไฟล์) ถ้ายังไม่มีไฟล์ พิมพ์อธิบายลักษณะงานมาแทนได้ค่ะ',
...
function askProcesses(data) {
  ...
  const options = ['กัด (Milling)', 'กลึง (Turning)', 'เจียร (Grinding)', 'เจาะ Gun Drill'].filter(
    (p) => !chosen.includes(p)
  );
...
function summaryText(data) {
  return [
    ...
    `ไฟล์/รายละเอียดงาน: ${data.drawingFileUrl || data.notes || '-'}`,
    `วัสดุ: ${data.material || '-'}`,
...
async function handleStep(userId, step, data, text, messageEvent) {
  switch (step) {
    case 'AWAIT_DRAWING': {
      ...
      const fileUrl =
        message.type === 'image' || message.type === 'file'
          ...
          : '';
      const next = { ...data, drawingFileUrl: fileUrl, notes: fileUrl ? '' : text };
      return askMaterial(next);
    }
    ...
    case 'AWAIT_PROCESSES': {
      if (text.trim() === 'เสร็จแล้ว') {
        return askQuantity(data);
      }
      const chosen = data.processes || [];
      if (!chosen.includes(text.trim())) {
        chosen.push(text.trim());
      }
