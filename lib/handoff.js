...
async function notifyAdmin(userId, userMessage) {
  ...
  // Never let a failure here (bad token, LINE outage, etc.) block clearing
  // the session or replying to the customer — the customer-facing handoff
  // reply and the admin ping are independent concerns.
  try {
    ...
    log.error('handoff.notify_admin_failed', { userId, error: err.message });
  }
}
